...
class FrequencyAssignmentGUI:
    ...
    def create_widgets(self):
        # Centered Title
        title_label = tk.Label(self.root, text="Frequency Assignment GUI", font=("Helvetica", 16))
    ...
    def solve_frequency_assignment(self):
        ...
        if result:
            ...
            message = (
                f"Number of channels needed: {num_channels}\n"
                f"Channel assignment for each station: {channel_assignment}"
            )
            self.result_label.config(text=message)

            self.plot_graph(graph, result)

            # Disable Solve Button after clicking
